...
describe "country post list sorting with ajax" do
  before :each do
    @user_2 = create(:user, :username => 'User Two', :email => 'different@email.com')
    @country = create(:country)
    @post_1 = create(:country_post, :content => 'first post', :commentable => @country)
    @post_2 = create(:country_post, :content => 'second post', :user => @user_2, :commentable => @country)
  end
  context "when user is not logged in" do
    context "on initial Country Detail page load" do

      it "displays post list in Last Updated DESC order" do
        visit country_path(@post_1.commentable)
